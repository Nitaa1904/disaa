<%-- 
    Document   : viewwisata
    Created on : Jan 8, 2024, 1:28:37 AM
    Author     : Nita
--%>
...
<body>
    ...
    <%@page import="com.javatpoint.dao.WisataDao,com.javatpoint.bean.*,java.util.*"%>  
    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>  
    <div class="container">
        </br></br></br></br>
        <h3 class="mb-5 mt-5" style="color: #439A97;">Pilih Data yang Ingin Dimodifikasi</h3>  
        <%  
        List<Wisata> list=WisataDao.getAllRecords();  
        request.setAttribute("list",list);  
        %>  
        <table border="1" width="90%" class="text-center">  
        <tr><th>Id</th><th>Nama</th><th>Tempat</th>  
        <th>Deskripsi</th><th>Edit</th><th>Delete</th></tr>  
        <c:forEach items="${list}" var="u">  
        <tr><td>${u.getId()}</td><td>${u.getNama()}</td>  
        <td>${u.getTempat()}</td><td>${u.getDeskripsi()}</td>  
        <td><a class="mt-2 mb-2" href="editform-wisata.jsp?id=${u.getId()}" 
               style="display: inline-block; padding: 10px 20px; background-color: #439A97; 
               color: #ffffff; text-decoration: none; text-align: center; border-radius: 5px;">Edit</a></td>  
        <td><a href="deletewisata.jsp?id=${u.getId()}" style="display: inline-block; padding: 10px 20px; 
               background-color: #BF3131; color: #ffffff; 
               text-decoration: none; text-align: center; border-radius: 5px;">Delete</a></td></tr>  
        </c:forEach>  
        </table>  
        <br/><a href="kumpulan wisata.jsp" style="display: inline-block; padding: 10px 20px; 
                background-color: #439A97; color: #ffffff; 
                text-decoration: none; text-align: center; border-radius: 5px;">Kembali</a><br/>  
    </div>
    <p class="mt-5"></br></br>
    <footer>
